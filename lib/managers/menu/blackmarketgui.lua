-- chunkname: lib/managers/menu/blackmarketgui.lua
	...
	for i, instance_id in ipairs(new_givens) do
		...
		elseif inventory_tradable[instance_id] then
			...
			if ti_td then
				...
				var_200_0 = params.item.category == "safes" and {
...
function BlackMarketGui:remove_mod_callback(data)
	...
	local var_338_0 = managers.blackmarket
	local var_338_1 = var_338_0
	local var_338_2 = var_338_0.get_modify_weapon_consequence
	local var_338_3 = data.category
	local var_338_4 = data.slot
	local var_338_5 = data.default_mod or data.name

	if data.default_mod then
		-- block empty
	end

	local replaces, removes = var_338_2(var_338_1, var_338_3, var_338_4, var_338_5, true)
	local weapon_id = managers.blackmarket:get_crafted_category(data.category)[data.slot].weapon_id
	local cost = managers.money:get_weapon_modify_price(weapon_id, data.name, data.global_value) or 0
